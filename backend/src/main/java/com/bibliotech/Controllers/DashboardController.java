...
public class DashboardController {
    ...
    public ResponseEntity<DashboardDTO> obterDashboard() {
        ...
        long emprestimosAtrasados = emprestimoRepository.findAll().stream()
                ...
                .count();
        long livrosDevolvidos = emprestimoRepository.findAll().stream()
                .filter(e -> "devolvido".equalsIgnoreCase(e.getStatus()))
                .count();

        DashboardDTO dto = new DashboardDTO(totalLivros, usuariosCadastrados, emprestimosAtivos, emprestimosAtrasados, livrosDevolvidos);
